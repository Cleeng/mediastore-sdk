...
import { MESSAGE_TYPE_SUCCESS, MESSAGE_TYPE_FAIL } from 'components/Input/InputConstants';
...
const couponInputProps = (value = '', showMessage = true, message = '', messageType = messageSuccess, fullWidth = false) => {
  return {
    value,
    fullWidth,
    couponDetails: {
...
describe('CouponInput component', () => {
  ...
  test('show success message', async () => {
    ...
    expect(screen.getByText('Success')).toHaveStyle(`color: ${Colors.ConfirmColor}`);
  });

  test('show error message', async () => {
    render(<CouponInput {...couponInputProps('', true, 'Error', messageFail)} />);
    await userEvent.click(screen.getByTestId('redeem-btn'));
    expect(screen.getByText('Error')).toHaveStyle(`color: ${Colors.ErrorColor}`);
  });

  test('check input with fullWidth props', async () => {
    render(<CouponInput {...couponInputProps('', false, '', messageSuccess, true)} />);
    expect(screen.getByTestId('inputcomponent')).toHaveStyle(`max-width: 300px;`)
    await userEvent.click(screen.getByTestId('redeem-btn'));
    expect(screen.getByTestId('inputcomponent')).toHaveStyle(`max-width: 100%`)
  })

  test('check input without fullWidth props', async () => {
    render(<CouponInput {...couponInputProps('', false, '', messageSuccess, false)} />);
    await userEvent.click(screen.getByTestId('redeem-btn'));
    expect(screen.getByTestId('inputcomponent')).toHaveStyle(`max-width: 300px;`)
  })
})
