...
| `MSSDK:Adyen-error`                  | `{error: "Error message", fieldType: "fieldType"}`                                       | The event will be emitted when any of the Adyen errors occur. The information about the error and the fieldType will be available in the event detail. The event will also be emitted when the error disappears after the first successful input verification. The error, in this case, will be an empty string. |
